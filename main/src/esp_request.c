...
static int nossl_connect(request_t *req)
{
    ...
    REQ_CHECK(sockett < 0, "socket failed", return -1);
    ...
    tv.tv_usec = 0;
    setsockopt(sockett, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

    ESP_LOGI(REQ_TAG, "[sock=%d],connecting to server IP:%s,Port:%s...",
             sockett, ipaddr_ntoa((const ip_addr_t*)&remote_ip.sin_addr.s_addr), (char*)port->value);
    if(connect(sockett, (struct sockaddr *)(&remote_ip), sizeof(struct sockaddr)) != 0) {
        close(sockett);
        return -1;
    }
    req->socket = sockett;
